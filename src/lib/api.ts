...
export const api = {
  ...
  addWallet: (address: string, label: string) => fetch(`${API_BASE}/api/wallets`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ address, label }) }).then(async r => { if (!r.ok) { const e = await r.json(); throw new Error(e.detail || 'Failed'); } return r.json(); }),
  ...
  getAlerts: () => fetch(`${API_BASE}/api/alerts?limit=50`).then(r => r.json()),
